...
const classes = Array.from({ length: 3 }, (_, grade) =>
...
export function ProfileSetupPage({ account, saving, error, onSave, onLogout }: Props) {
  ...
  const [className, setClassName] = useState('2年3組')
    ...
    <main className="login-page profile-setup-page">
      ...
      <section className="login-panel">
        <form className="login-card profile-form" onSubmit={submit} noValidate>
          ...
          <fieldset className="role-selector" disabled={saving}>
            <legend>利用区分</legend>
          ...
          <div className="field">
            ...
            <select id="profile-class" value={className} onChange={(event) => setClassName(event.target.value)} disabled={saving}>{classes.map((item) => <option key={item}>{item}</option>)}</select>
          </div>
